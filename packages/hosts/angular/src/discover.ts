...
function discoverTagFiles(
  ...
  // Core *throws* on a host module file under `tags/` (`scan.ts`'s
  // `rejectHostModuleFile`, added by PR #108 after this host's own handling
  // of the same case was written, so the two never met). This host already
  // has the friendlier message, so the throw is converted to it here rather
  // than in core, where `.solid.mx` under `tags/` must stay fatal for the
  // hosts that have no component-module route at all.
  //
  // The file is then routed *nowhere*: it is neither a tag (core said so)
  // nor something to compile as a component module, because a `tags/`
  // directory is not where a component module belongs. Compiling it anyway
  // would write a `.ts` beside it that the author never asked for.
